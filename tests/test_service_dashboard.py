import gc, os, tempfile, time, unittest
...
class DashboardTests(unittest.TestCase):
    ...
    def tearDown(self):
        # sqlite context managers commit/rollback but connection finalization can lag on Windows.
        gc.collect()
        for p in (self.path,self.path+"-wal",self.path+"-shm"):
            for _ in range(5):
                try: os.remove(p); break
                except FileNotFoundError: break
                except PermissionError: gc.collect(); time.sleep(.05)
